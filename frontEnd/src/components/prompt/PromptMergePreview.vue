...
const showPosFull = ref(false)
const showNegFull = ref(false)
...
<template>
  <div
    data-prompt-merge-preview
    class="rounded-lg border border-dashed border-primary/35 bg-primary/5 overflow-hidden"
  >
    <button
      type="button"
    ...
    >
      <div class="min-w-0">
        <Label class="pointer-events-none text-sm font-medium">合并预览（将提交给 ComfyUI）</Label>
        <p v-if="!expanded" class="mt-0.5 truncate text-xs text-muted-foreground">
          点击展开查看全局 / 随机 / 工作流合并结果
        </p>
      </div>
      <ChevronDown
    ...
    <AnimatedCollapse v-model="expanded">
      <div class="space-y-2 border-t border-primary/20 px-3 pb-3 pt-2">
        <div class="flex items-center justify-end">
          <Button variant="outline" size="sm" class="h-8 text-xs" :disabled="loading" @click="refresh">
            {{ loading ? '刷新中…' : '刷新预览' }}
          </Button>
        </div>
        ...
        <div class="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
          ...
          <span class="inline-flex items-center gap-1">
            <span class="h-2.5 w-2.5 rounded border border-sky-500/35 bg-sky-500/10" />
            工作流与当次
          </span>
          <span class="text-muted-foreground/80">· 色块顺序即拼接顺序；提交前会去重</span>
        </div>

        <p v-if="debug" class="text-xs text-muted-foreground">
        ...
        <div v-if="loading && !hasLoaded" class="py-4 text-center text-sm text-muted-foreground">
          加载预览…
        </div>

        <div v-else class="grid gap-2 lg:grid-cols-2">
          <div>
            <p class="mb-1 text-sm font-medium text-muted-foreground">正向</p>
            <div class="max-h-52 space-y-2 overflow-auto rounded bg-background/50 p-2">
              <PromptMergeSegmentBlocks :segments="segments.positive" side="positive" />
            </div>
            <div v-if="positive" class="mt-1">
              <button
                type="button"
                class="text-xs text-muted-foreground transition-colors hover:text-foreground"
                @click="showPosFull = !showPosFull"
              >
                {{ showPosFull ? '收起' : '查看' }}去重后完整正向
              </button>
              <AnimatedCollapse v-model="showPosFull">
                <pre
                  class="mt-1 max-h-32 overflow-auto whitespace-pre-wrap rounded border border-input/60 bg-background/90 p-2.5 text-sm leading-relaxed"
                  >{{ positive }}</pre
                >
              </AnimatedCollapse>
            </div>
          ...
          <div>
            <p class="mb-1 text-sm font-medium text-muted-foreground">负向</p>
            <div class="max-h-48 space-y-2 overflow-auto rounded bg-background/50 p-2">
              <PromptMergeSegmentBlocks :segments="segments.negative" side="negative" />
            </div>
            <div v-if="negative" class="mt-1">
              <button
                type="button"
                class="text-xs text-muted-foreground transition-colors hover:text-foreground"
                @click="showNegFull = !showNegFull"
              >
                {{ showNegFull ? '收起' : '查看' }}去重后完整负向
              </button>
              <AnimatedCollapse v-model="showNegFull">
                <pre
                  class="mt-1 max-h-28 overflow-auto whitespace-pre-wrap rounded border border-input/60 bg-background/90 p-2.5 text-sm leading-relaxed"
                  >{{ negative }}</pre
                >
              </AnimatedCollapse>
            </div>
